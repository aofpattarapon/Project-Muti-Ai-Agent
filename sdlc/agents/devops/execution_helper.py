...
def generate_release_notes(result: dict, task_id: str, project_name: str = "") -> tuple:
    """
    Generate release_notes.md content from deployment check results.

    Returns (content_str, status_str) where status_str is one of:
      "generated" — checks passed; notes list ready checks
      "skipped"   — blocked or no stack detected; notes explain why
    """
    status = result.get("status", "skipped")
    stack = result.get("detected_stack", [])
    now = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")

    # Skip if blocked or no stack (nothing useful to say about readiness)
    if status == "blocked" or not stack:
        content = (
            f"# Release Notes — {project_name or 'N/A'}\n"
            f"**Task:** `{task_id}`  |  **Date:** {now}\n\n"
            f"**release_notes_status:** skipped\n\n"
        )
        if status == "blocked":
            blockers = result.get("deployment_blockers", [])
            content += "Release notes generation skipped — deployment checks were blocked by infrastructure or tooling issues.\n\n"
            if blockers:
                content += "**Blockers (resolve before release):**\n"
                for b in blockers[:5]:
                    content += f"- 🚫 {b}\n"
        else:
            content += "Release notes generation skipped — no recognized stack detected in workspace.\n"
        content += "\nManual approval required before deployment.\n"
        return content, "skipped"

    # Generate notes for passed/failed (stack detected, not blocked)
    passed = [c for c in result.get("checks", []) if c["status"] == "passed"]
    ...
    lines = [
        ...
        f"**Task:** `{task_id}`  |  **Date:** {now}",
        f"**Stack:** {', '.join(stack)}",
        f"**Deployment Readiness:** {status.upper()}",
        "",
    ...
    if passed:
        ...
        for c in passed:
            lines.append(f"- {c['name']}: {(c.get('stdout_tail') or 'ok')[:100]}")
        lines.append("")

    if failed:
        lines.append("## ❌ Issues Found (must resolve before release)")
        for c in failed:
            tail = (c.get("stderr_tail") or c.get("stdout_tail") or c.get("skip_reason") or "")[:100]
            lines.append(f"- {c['name']}: {tail}")
        lines.append("")

    lines += [
        "## Deployment Approval",
        "Manual approval required before any production deployment.",
        "All blockers listed above must be resolved first.",
        "",
        f"*Generated by DEVOPS Agent — {now}*",
    ]
    return "\n".join(lines) + "\n", "generated"
...
class DEVOPSExecutionHelper:
    ...
    def save_execution_artifact(
        ...
        """Write devops_execution_result.json, deployment_readiness_report.md, release_notes.md."""
        try:
            ...
            release_notes, notes_status = generate_release_notes(result, task.id, project_name)
            notes_path = os.path.join(output_dir, "release_notes.md")
            with open(notes_path, "w", encoding="utf-8") as fh:
                fh.write(release_notes)
            logger.debug(f"[devops] release_notes: {notes_status} → {notes_path}")
