...
class BaseAgent(ABC):
    ...
    async def _execute_sdlc_task(self, task: SdlcTask, guild: discord.Guild):
        ...
        # Runs after post_save_hook so secondary artifacts (manifests, reports)
        # generated by the hook are on disk and checkable.
        ...
        if _contract.status == "failed":
            _fail_count = (self._task_fail_counts.get(task.id, 0) + 1)
            self._task_fail_counts[task.id] = _fail_count
            _requeue = _fail_count <= _MAX_AUTO_RETRIES
            logger.warning(
