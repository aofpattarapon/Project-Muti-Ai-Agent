...
import { createAgentActivityLog, createAgentRoleConfig } from "@/lib/agents/query";
...
export async function POST(request: Request) {
  ...
  const roleKey = String(formData.get("role_key") ?? "").trim().toLowerCase();
  ...
  const created = createAgentRoleConfig({
    roleKey,
    displayName,
  ...
  if (created) {
    recordAuditEvent(
      ...
      `Agent role config ${roleKey} created.`,
      session.username,
    );
    ...
    createAgentActivityLog({
      ...
      status: "created",
      summary: `Role config ${displayName} was created from the project back-office.`,
